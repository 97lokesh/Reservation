import  daoUser from "../daos/user.js"
...
const getLoginDetails = async function getLoginDetails (queryFields) {
  const loginFields = {
    name: 1,
    salt: 1,
    iterations: 1,
...
const loginUser = async function loginUser(body) {
...
const logoutUser = async function logoutUser(body) {
  if (!body.hasOwnProperty("email")) {
    return { success: false, error: "missing email" };
  }
